...
export function resolveLocalHelp(home: HomeProfile | null): LocalHelpResolved {
  ...
  if (!placeKnown) {
    missingNote =
      "Add an address or ZIP in setup so FaultLine can attach state and county numbers. National lines still work.";
  } else if (state && !STATE_HELP[state]) {
    missingNote =
      "This state is not in the published office table. FaultLine will not invent a local number - use 211 or Ready.gov to find your emergency-management office.";
    ...
    missingNote =
      `No published county line is stored for ${county} County. Use 211 or the state office - FaultLine will not invent one.`;
  }
